...
/* helper function to print only non-zero memory locations */
void print_non_zero_memory(Memory * mem, const char * title)
{
	fprintf(stderr, "\n=== %s ===\n", title);
	int count = 0;
	int items_per_line = 8;
	int items_in_current_line = 0;

	for (int i = 0; i < MEM_SIZE; i++) {
		if (mem->data[i] != 0) {
			/* Start new line if needed */
			if (items_in_current_line == 0) {
				fprintf(stderr, " ");
			}

			/* Print the memory location and value */
			fprintf(stderr, "[%05d]:%11ld", i, mem->data[i]);
			count++;
			items_in_current_line++;

			/* Add tab separator or newline */
			if (items_in_current_line < items_per_line) {
				fprintf(stderr, "\t");
			} else {
				fprintf(stderr, "\n");
	...
	/* End the last line if it wasn't completed */
	if (items_in_current_line > 0) {
		fprintf(stderr, "\n");
	}

	if (count == 0) {
		fprintf(stderr, "No non-zero memory locations found.\n");
	} else {
		fprintf(stderr, "Total non-zero locations: %d\n", count);
	}
...
int all_user_threads_terminated(Memory * mem)
{
	...
	for (int thread_id = 2; thread_id <= 7; thread_id++) {
		long int entry_base = thread_table_base + (thread_id * 8);
		long int state = mem_read(mem, entry_base + 1, KERNEL);
...
int parse_arguments(int argc, char * argv[], SimulationConfig * config)
{
	...
	config->max_instructions = 20000000;  /* increased default limit for thread completion */
...
int main(int argc, char * argv[])
{
	...
	/* Initialize CPU */
	CPU cpu;
	cpu_init(&cpu, &mem, config.debug_mode);
	...
	{
		mem_dump(&mem, 4000, 32);
	}
	/* main execution loop */
	while (!cpu_is_halted(&cpu) && instruction_count < config.max_instructions)
	{
		/* For OS programs: check if all user threads are done (only idle thread left) */
		if (is_os_program && !is_single_thread_program && all_user_threads_terminated(&mem)) {
		...
		/* get current PC for syscall detection (only for OS programs) */
		long int current_pc = mem_read(&mem, REG_PC, KERNEL);
		int is_syscall = (is_os_program && !is_single_thread_program) ? is_syscall_instruction(&mem, current_pc) : 0;

		/* detect context switch (only for OS programs) */
		int context_switched = (cpu.curr_thread_id != last_thread_id);
		if (context_switched && is_os_program && !is_single_thread_program) {
			context_switches++;
		...
		/* debug mode 3: Print thread table before syscall (only for OS programs) */
		if (config.debug_mode == DEBUG_MODE_3 && is_syscall && is_os_program && !is_single_thread_program) {
		...
		/* debug mode 1: Non-zero memory dump after each instruction */
		if (config.debug_mode == DEBUG_MODE_1) {
			char title[256];
			snprintf(title, sizeof(title), "INSTRUCTION %d - Non-Zero Memory (Thread: %d, Mode: %s, PC: %ld)",
					instruction_count,
					cpu.curr_thread_id,
					cpu.mode == KERNEL ? "KERNEL" : "USER",
					mem_read(&mem, REG_PC, KERNEL));
			print_non_zero_memory(&mem, title);
		}

		/* debug mode 2: Non-zero memory dump + wait for keypress */
		...
		/* debug mode 3: Print thread table after syscall execution (only for OS programs) */
		if (config.debug_mode == DEBUG_MODE_3 && is_syscall && is_os_program && !is_single_thread_program) {
			fprintf(stderr, "\n[INSTRUCTION %d] THREAD TABLE AFTER SYSCALL:\n", instruction_count);
			//print_thread_table(&mem);
		}

		/* Count syscalls (only for OS programs) */
		if (is_syscall && is_os_program && !is_single_thread_program) {
			syscalls++;
		}
	...
	/* debug mode 0: Non-zero memory dump after CPU halts */
	if (config.debug_mode == DEBUG_MODE_0) {
		char title[256];
		snprintf(title, sizeof(title), "FINAL NON-ZERO MEMORY DUMP (Debug Mode 0) - Thread: %d, Mode: %s, PC: %ld",
				cpu.curr_thread_id,
				cpu.mode == KERNEL ? "KERNEL" : "USER",
				mem_read(&mem, REG_PC, KERNEL));
		print_non_zero_memory(&mem, title);
	...
	/* final thread table for mode 3 */
	if (config.debug_mode == DEBUG_MODE_3 && !is_single_thread_program) {
		fprintf(stderr, "\n=== FINAL THREAD TABLE ===\n");
		print_thread_table(&mem);
	}
